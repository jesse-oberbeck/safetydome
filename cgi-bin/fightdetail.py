...
def populate(connection, one, two):
    ...
    sql = "SELECT * FROM fight WHERE combatant_one = '" + str(one) + "' AND \
          combatant_two = '" + str(two) + "';"
    cursor = connection.cursor()
    cursor.execute(sql)
    data = cursor.fetchall()[0]

    comb1 = cursor.execute("SELECT name FROM combatant WHERE id = " +
                           str(data[0]))
    comb1 = cursor.fetchall()[0][0]
    comb2 = cursor.execute("SELECT name FROM combatant WHERE id = " +
                           str(data[1]))
    comb2 = cursor.fetchall()[0][0]
    winner = cursor.execute("SELECT name FROM combatant WHERE id = " +
                            str(data[2]))
    ...
    # Table header
    print("<tr>")
    print("<th>Combatant 1</th> <th>Combatant 2</th> <th>Winner</th>\
